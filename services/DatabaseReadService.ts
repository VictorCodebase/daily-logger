...
// It is good practice to define interfaces for your data models.
// You can keep them here or move them to a separate models.ts file later.
...
export async function dayExists(date: string): Promise<number | null> {
    try {
        const db = await setupDatabase();
        const result = await db.getFirstAsync<{ day_id: number }>(
...
export async function dayActivitiesExist(day_id: number): Promise<number[] | null> {
    try {
        const db = await setupDatabase();
        const results = await db.getAllAsync<{ activity_id: number }>(
            `SELECT activity_id FROM Activity WHERE day_id = ?;`,
            [day_id]
        );
...
export async function daySpecialActivitiesExist(day_id: number): Promise<number[] | null> {
    try {
        const db = await setupDatabase();
        const results = await db.getAllAsync<{ sp_activity_id: number }>(
            `SELECT sp_activity_id FROM Special_Activity WHERE day_id = ?;`,
            [day_id]
        );
...
export async function logTemplatesExist(): Promise<{ log_template_id: number; name: string }[] | null> {
    try {
        const db = await setupDatabase();
        const results = await db.getAllAsync<{ log_template_id: number; name: string }>(
            `SELECT log_template_id, name FROM Log_Template;`
        );
...
export async function exportTemplatesExist(): Promise<{ export_template_id: number; name: string }[] | null> {
    try {
        const db = await setupDatabase();
        const results = await db.getAllAsync<{ export_template_id: number; name: string }>(
            `SELECT export_template_id, name FROM Export_Template;`
        );
        return results.length > 0 ? results : null;
    } catch (error) {
...
export async function readDay(day_id: number): Promise<{
    ...
    try {
        const db = await setupDatabase();
        const dayResult = await db.getFirstAsync<Day>(
        ...
        const specialActivities = await db.getAllAsync<SpecialActivity>(
            `SELECT * FROM Special_Activity WHERE day_id = ?;`,
            [day_id]
        );
...
export async function readDays(): Promise<{ day_id: number, date: string }[] | null> {
    try {
        const db = await setupDatabase();
        const results = await db.getAllAsync<{ day_id: number, date: string }>(
            `SELECT day_id, date FROM Day;`
        );
        return results.length > 0 ? results : null;
    } catch (error) {
...
export async function readLogTemplate(log_template_id: number): Promise<LogTemplate | null> {
    try {
        const db = await setupDatabase();
        const result = await db.getFirstAsync<LogTemplate>(
            `SELECT * FROM Log_Template WHERE log_template_id = ?;`,
            [log_template_id]
        );
        return result || null;
    } catch (error) {
...
export async function readExportTemplate(export_template_id: number): Promise<ExportTemplate | null> {
    try {
        const db = await setupDatabase();
        const result = await db.getFirstAsync<ExportTemplate>(
            `SELECT * FROM Export_Template WHERE export_template_id = ?;`,
            [export_template_id]
        );
        return result || null;
    } catch (error) {
